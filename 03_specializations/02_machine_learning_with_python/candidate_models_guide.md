...
5. **Features must be numerical** and on the same scale. Can still be used with `StandardScaler` before clustering; cannot remain a candidate model for categorical features — use K-Modes or K-Prototypes instead.
...
3. Does **not** assume convex shapes, does **not** need `k` pre-specified, and handles outliers natively.
