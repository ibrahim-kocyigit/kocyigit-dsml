...
⚠️ **Main risk:** High variance (overfitting) *(Can still be used if/with pruning, depth limits, or switching to ensemble methods like Random Forest)*
...
**Assumptions:**
1. **Linear separability** (for linear SVM) *(Can still be used if/with the Kernel Trick — RBF, Polynomial kernels — to handle non-linear boundaries)*
...
2. **Weak learners are slightly better than random** — each base model (typically a shallow tree) must have >50% accuracy *(Can still be used if/with increasing the number of boosting rounds or tuning learning rate)*
...
3. **Dataset is not too large** — $O(N^2)$ time complexity makes it very slow on large datasets *(Can still be used if/with subsampling the data or using Barnes-Hut approximation for speedup)*
