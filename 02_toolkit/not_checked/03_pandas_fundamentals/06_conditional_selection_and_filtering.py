# 06_conditional_selection_and_filtering.py
...
if not df.empty:
    ...
    high_value_electronics = df[
        (df['Category'] == 'Electronics') & (df['Price'] > 100)
    ]
    print("High-value electronics (Category == 'Electronics' AND Price > 100):\n", high_value_electronics)


    # --- Example with `|` (OR) ---
    # Find all sales that are either a 'Laptop' OR have a quantity of 3 or more.
    laptops_or_high_qty = df[
        (df['Product'] == 'Laptop') | (df['Quantity'] >= 3)
    ]
...
if not df.empty:
    ...
    str_contains_df = df[df['Product'].str.contains('o', case=False)] # case=False makes it case-insensitive
...
if not df.empty:
    ...
    print("Result of `df.query('Category == \"Electronics\" and Price > 100')`:\n", query_result)
