...
data_points[2] = 30
print(f"After changing index 2: {data_points}")  # [10, 20, 30, 40, 50]

# Change a range of elements
data_points[3:5] = [45, 55, 65]
print(f"After changing a slice: {data_points}")  # [10, 20, 30, 45, 55, 65]
...
# - `.extend(iterable)`: Appends all items from another list (or iterable).
...
print(f"Remaining tasks: {tasks}")  # ['do laundry', 'buy groceries', 'call mom']
...
letters = ["a", "b", "c", "a", "a", "d", "a"]
print(f"Index of 'c': {letters.index('c')}")  # Index of 'c': 2
print(f"Count of 'a': {letters.count('a')}")  # Count of 'a': 4
...
correct_copy[0] = 100
print(f"Correct copy: {correct_copy}")  # This shows the independent modification
print(
