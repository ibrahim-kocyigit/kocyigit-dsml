...
if not df.empty:
    ...
    aggregations = df.groupby('Category').agg(
        # Pass a dictionary where keys are columns and values are the functions to apply
        {
            ...
            'TotalPrice': ['sum', 'max'] # Apply multiple functions to the TotalPrice column
        }
    )
